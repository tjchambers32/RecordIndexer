 ...
 * 
 */
package main.java.client.gui.batchstate;
...
import main.java.client.communication.ClientCommunicator;
import main.java.client.ClientException;
import main.java.shared.communication.DownloadBatch_Result;
import main.java.shared.communication.SubmitBatch_Params;
import main.java.shared.communication.SubmitBatch_Result;
import main.java.shared.model.Field;
import main.java.shared.model.Record;
import main.java.shared.model.User;
import main.java.shared.model.Value;
...
public class BatchState implements BatchStateListener {
	...
	public boolean submitBatch() {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Could not submit batch.", "Submit Batch Error",
	...
	public boolean qualityCheck(Cell cell) {
		...
		// if there isn't a dictionary for the field, we can't mark it as
		// incorrect
	...
	private Set<String> findNearbyWords(String word) {
		...
		for (int i = 0; i < word.length(); i++) {
			for (int j = 0; j < 26; j++) {
				sb = new StringBuilder(word);
				char inputChar = (char) (j + 'a'); // inputChar matching
													// original char doesn't
													// matter because that would
													// mean our original word is
													// in the Trie, which we
													// already checked for
